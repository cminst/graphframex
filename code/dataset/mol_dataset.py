...
def extract_gz(path, folder, log=True):
    ...
    with gzip.open(path, "r") as r:
        with open(
            osp.join(folder, ".".join(os.path.basename(path).split(".")[:-1])), "wb"
        ) as w:
            w.write(r.read())
...
class MoleculeDataset(InMemoryDataset):
    ...
    names = {
        ...
        "pcba": ["PCBA", "pcba.csv.gz", "pcba.csv", -1, slice(0, 128)],
        "muv": ["MUV", "muv.csv.gz", "muv.csv", -1, slice(0, 17)],
        "hiv": ["HIV", "HIV.csv", "HIV.csv", 0, -1],
        "bace": ["BACE", "bace.csv", "bace.csv", 0, 2],
        "bbbp": ["BBBP", "BBBP.csv", "BBBP.csv", -1, -2],
        "tox21": ["Tox21", "tox21.csv.gz", "tox21.csv", -1, slice(0, 12)],
        "toxcast": [
            "ToxCast",
            "toxcast_data.csv.gz",
            "toxcast_data.csv",
        ...
        "sider": ["SIDER", "sider.csv.gz", "sider.csv", 0, slice(1, 28)],
        "clintox": ["ClinTox", "clintox.csv.gz", "clintox.csv", 0, slice(1, 3)],
    }
    ...
    def process(self):
        if self.name.lower() == "MUTAG".lower():
            ...
            with open(
                os.path.join(self.raw_dir, "MUTAG_graph_indicator.txt"), "r"
            ) as f:
